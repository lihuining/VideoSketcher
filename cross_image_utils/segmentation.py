...
class Segmentor:
    def __init__(self, prompt: str, object_nouns: List[str],config, num_segments: int = 5, res: int = 32):
    ...
    def update_attention(self, attn, is_cross):
        ...
        res = int(attn.shape[2] ** 0.5) # attn_weight:(3,8,1024,1024)
        if is_cross:
            if res == 16:
...
# {0: (4, 'object'), 1: 'BG', 2: (4, 'object'), 3: 'BG', 4: (4, 'object')}
    ...
    def get_object_masks(self) -> Tuple[torch.Tensor]:
        ...
        show_cluster_image(save_dir_cluster, clusters_struct_32, 'struct_32', segments=self.num_segments)
        show_cluster_image(save_dir_cluster, clusters_style_32, 'style_32', segments=self.num_segments)
        ...
        save_dir = str(self.mask_path)
        show_tensor_image(save_dir, mask_style_64, 'style', 64)
        show_tensor_image(save_dir, mask_style_32, 'style', 32)
        show_tensor_image(save_dir, mask_struct_32, 'struct', 32)
