...
def initialize_ted(device):
    model = TED()
    checkpoint_path = 'TEED/checkpoints/BIPED/7/7_model.pth'
...
def initialize_hed(device):
    ...
    hed_checkpoint = torch.load('./checkpoint/network-bsds500.pytorch', map_location=device)
...
if __name__ == "__main__":
    image_path = "/media/allenyljiang/5234E69834E67DFB/Dataset/Video_Dataset/DAVIS-2017-trainval-Full-Resolution/DAVIS/dataset/breakdance-flare/imgs_crop_fore/00000.jpg"
    ...
    for method in method_list:
        save_path = os.path.join(save_dir, f"{name}_{method}.png")
        load_edge(image_path, device="cuda", method=method, alpha=0.55, save_path=save_path)