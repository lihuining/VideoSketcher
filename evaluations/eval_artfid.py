...
def compute_fid(total_frames,path_to_stylized, path_to_style, batch_size, device, num_workers=1,art_inception_model=''):
    ...
    if not art_inception_model:
        ...
        model = art_inception_model
    
    stylized_image_paths = get_image_paths(path_to_stylized)[:total_frames]
...
def compute_video_fid_infinity(total_frames,path_to_stylized, path_to_style, batch_size, device, num_points=15, num_workers=1,art_inception_model=''):
    ...
    stylized_image_paths = get_image_paths(path_to_stylized)[:total_frames]
    style_image_paths = [path_to_style]
    ...
    # repeat
    activations_style = np.repeat(activations_style, activations_stylized.shape[0], axis=0)
    ...
    return fid_infinity
def compute_fid_infinity(total_frames,path_to_stylized,art_inception_model, path_to_style, batch_size, device, num_points=15, num_workers=1):
    """Computes the FID infinity for the given paths.
...
def compute_content_distance(total_frames,path_to_stylized, path_to_content, batch_size, content_metric='lpips', device='cuda', num_workers=1, gray=False):
    ...
    stylized_image_paths = get_image_paths(path_to_stylized, sort=True)[:total_frames]
    content_image_paths = get_image_paths(path_to_content, sort=True)[:total_frames]
...
def compute_patch_simi(total_frames,path_to_stylized, path_to_content, batch_size, device, num_workers=1):
    """Computes the distance for the given paths.
    ...
    stylized_image_paths = get_image_paths(path_to_stylized, sort=True)[:total_frames]
    content_image_paths = get_image_paths(path_to_content, sort=True)[:total_frames]
    print("stylized_image_paths",len(stylized_image_paths),"content_image_paths",len(content_image_paths))
    assert len(stylized_image_paths) == len(content_image_paths), \
           'Number of stylized images and number of content images must be equal.'
    ...
    pbar.close()
    if len(stylized_image_paths) == 0 or len(content_image_paths) == 0:
        print("Warning: No stylized or content images found for the current video and style.")
        return 0
    else:
        return dist_sum / N

def compute_art_fid(path_to_stylized, path_to_style, path_to_content,total_frames=50,art_inception_model='',device='cuda',batch_size=1, mode='art_fid_inf', content_metric='lpips', num_workers=8):
    """Computes the FID for the given paths.
    ...
    Args:
        ...
        batch_size (int): Batch size for computing activations.
        device (str): Device for commputing activations.
        content_metric (str): Metric to use for content distance. Choices: 'lpips', 'vgg', 'alexnet',['lpips', 'vgg', 'alexnet', 'ssim', 'ms-ssim']
        num_workers (int): Number of threads for data loading.
    ...
    total_frames = min(len(get_image_paths(path_to_stylized)),total_frames)
    print("processing frame number is",total_frames)
    print('Compute FID value...')
    if mode == 'art_fid_inf':
        fid_value = compute_video_fid_infinity(total_frames,path_to_stylized, path_to_style, batch_size, device, num_workers=num_workers,art_inception_model=art_inception_model)
    elif mode == 'art_fid':
        fid_value = compute_fid(total_frames,path_to_stylized, path_to_style, batch_size, num_workers=num_workers,art_inception_model=art_inception_model)

    
    print('Compute content distance...')
    cnt_value = compute_content_distance(total_frames,path_to_stylized, path_to_content, batch_size, content_metric, device, num_workers)
    gray_cnt_value = compute_content_distance(total_frames,path_to_stylized, path_to_content, batch_size, content_metric, device, num_workers, gray=True)
...
def compute_cfsd(total_frames,path_to_stylized, path_to_content, device="cuda", num_workers=1):
    ...
    simi_val = compute_patch_simi(total_frames,path_to_stylized, path_to_content, 1, device, num_workers)
    simi_dist = simi_val.item()
...
def main():
    ...
    parser.add_argument('--total-frames', type=int, default=50, help='Path to stylized images.')
    args = parser.parse_args()
    if args.total_frames == 0:
        args.total_frames = len(get_image_paths(args.tar))
    print("total processing frames",args.total_frames)
    ...
    # cfsd = compute_cfsd(args.total_frames,
    #                     args.tar,
    #                     args.cnt,
    #                     args.batch_size,
    #                     args.device,
...
if __name__ == '__main__':
    artfid, fid, lpips, lpips_gray = compute_art_fid(tar,sty,cnt)
'''
cd evaluation;
#python eval_artfid.py --sty /media/allenyljiang/564AFA804AFA5BE51/Codes/single_frame_sketch_baselines/StyleID/data/sty --cnt /media/allenyljiang/564AFA804AFA5BE51/Codes/single_frame_sketch_baselines/StyleID/data/cnt --tar /media/allenyljiang/564AFA804AFA5BE51/Codes/single_frame_sketch_baselines/StyleID/output
python eval_artfid.py --sty /media/allenyljiang/5234E69834E67DFB/Dataset/Sketch_dataset/ref2sketch_yr/ref_camel/ref0020.jpg --cnt /media/allenyljiang/5234E69834E67DFB/Dataset/Video_Dataset/DAVIS-2017-trainval-Full-Resolution/DAVIS/dataset/goat/imgs_crop_fore --tar /media/allenyljiang/2CD8318DD83155F4/CVPR2025/Struct_latents/goat/ref0020/2.1_chunk_size2/generated_result
'''