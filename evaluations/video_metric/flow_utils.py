...
gmflow_dir = os.path.join(parent_dir, 'video_metric', 'deps', 'gmflow')
sys.path.insert(0, gmflow_dir)

from gmflow.gmflow import GMFlow  # noqa: E702 E402 F401
from gmflow_utils.utils import InputPadder  # noqa: E702 E402
...
def get_warped_and_mask(flow_model,
    ...
    image1, image2 = padder.pad(image1[None].cuda(), image2[None].cuda()) # (1,3,320,512)
    ...
    flow_pr = results_dict['flow_preds'][-1]  # list -> [B, 2, H, W] -> [2,2,320,512]
...
# # 1. 转换张量为 NumPy 格式
# bwd_occ_np = bwd_occ.squeeze().detach().cpu().numpy()
#
# # 2. 归一化张量值到 0-255 范围
...
# image.save('bwd_occ.png')

class FlowCalc():

    def __init__(self, model_path='/media/allenyljiang/564AFA804AFA5BE51/Codes/Video_Editing/Rerender_A_Video/models/gmflow_sintel-0c07dcb3.pth'):
        flow_model = GMFlow(
