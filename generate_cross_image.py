...
class Generator(nn.Module):
    def __init__(self, pipe, scheduler, config):
        ...
        self.batch_size = 2
        self.control = gene_config.control # 'depth'
        self.use_depth = config.sd_version == "depth"
        self.use_controlnet = self.control in CONTROLNET_DICT.keys() # 是否加controlnet
        self.use_pnp = self.control == "pnp"
        self.use_cross_attention = self.control == "cross_attention"
        if self.use_controlnet:
            self.controlnet = pipe.controlnet
            self.controlnet_scale = gene_config.control_scale # 1.0
        elif self.use_pnp:
            pnp_f_t = int(gene_config.n_timesteps * gene_config.pnp_f_t) # 50*0.8 = 40
            pnp_attn_t = int(gene_config.n_timesteps * gene_config.pnp_attn_t) # 50*0.5 = 25
            self.batch_size += 1
            self.init_pnp(conv_injection_t=pnp_f_t, qk_injection_t=pnp_attn_t)
        elif self.use_cross_attention:
            self.init_cross_image_attention()


        self.chunk_size = gene_config.chunk_size # 4
        self.chunk_ord = gene_config.chunk_ord # ‘mix-4’
        self.merge_global = gene_config.merge_global # True
        self.local_merge_ratio = gene_config.local_merge_ratio # 0.95
        self.global_merge_ratio = gene_config.global_merge_ratio # 0.9
        self.global_rand = gene_config.global_rand # 0.1
        self.align_batch = gene_config.align_batch # True
    ...
    def get_text_embeds_input(self, prompt, negative_prompt):
        ...
        if self.use_pnp:
            ...
            text_embeds = torch.cat(
                [pnp_guidance_embeds, text_embeds], dim=0) # torch.Size([3, 77, 768])
        return text_embeds
    ...
    def prepare_data(self, data_path, latent_path, frame_ids):
        ...
        self.init_noise = load_latent(
            latent_path, t=self.scheduler.timesteps[0], frame_ids=frame_ids).to(self.dtype).to(self.device) # torch.Size([64, 4, 64, 64])
    ...
    @torch.no_grad()
    def ddim_sample(self, x, conds): # 从噪声到clean x0
        ...
        for i, t in enumerate(tqdm(timesteps, desc="Sampling")):
            ...
            x = self.pred_next_x(x, noises, t, i, inversion=False) # torch.Size([64, 4, 64, 64])
    ...
    def pred_noise(self, x, cond, t, batch_idx=None):
        ...
        # Pred noise!
        eps = self.unet(latent_model_input, t, encoder_hidden_states=text_embed_input, **kwargs).sample # (12,4,64,64)
        noise_pred_uncond, noise_pred_cond = eps.chunk(batch_size)[-2:] # (4,4,64,64) (4,4,64,64)
        # CFG
    ...
    def pred_next_x(self, x, eps, t, i, inversion=False):
        ...
        return x
    def init_cross_image_attention(self,start):
        pass


    def init_pnp(self, conv_injection_t, qk_injection_t):
        qk_injection_timesteps = self.scheduler.timesteps[:qk_injection_t] if qk_injection_t >= 0 else [] # torch.Size([25])
        conv_injection_timesteps = self.scheduler.timesteps[:conv_injection_t] if conv_injection_t >= 0 else [] # torch.Size([40])
        register_attention_control(
            self, qk_injection_timesteps, num_inputs=self.batch_size) # batchsize = 3
...
'''
python generate.py --config configs/tea-pour-debug.yaml
'''