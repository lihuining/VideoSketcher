...
class Inverter(nn.Module):
        ...
        with torch.autocast(device_type=self.device, dtype=self.dtype):
            for i, t in enumerate(tqdm(timesteps)):
                ...
                for batch in batches:
                    ...
                    noises += [noise]
                noises = torch.cat(noises) # （64，4，64，64）
                x = self.pred_next_x(x, noises, t, i, inversion=True) #（64，4，64，64）
    ...
    def __call__(self, data_path, save_path):
        ...
        conds, prompts = self.prepare_cond(self.prompt, len(frames)) # conds:(64,77,768) prompts:list,64
