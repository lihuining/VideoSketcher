...
def register_attention_control(model, injection_schedule, num_inputs):
    def sa_forward(self):
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None, **kwargs):
            ...
            if not is_cross and self.injection_schedule is not None and (
                ...
                attn = attn.repeat(num_inputs, 1, 1) # num_inputs = 3 attn：（96，256，256）

            out = torch.einsum("b i j, b j d -> b i d", attn, v) # （96，256，160）
    ...
    res_dict = {1: [1, 2], 2: [0, 1, 2], 3: [0, 1, 2]} # cross:image-attention res_dict = {2: [0, 1, 2], 3: [0, 1, 2]}
    for res in res_dict:
